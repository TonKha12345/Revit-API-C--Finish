...
namespace AlphaBIM
{
    public static class FamilyUtils
    {
            ...
            string bPara, string hPara)
        {
            ...
            {
                System.Windows.MessageBox.Show("Two parameters dimemsion of column family have to name is"
                                + bPara + hPara);
                return null;
            }
            ...
            string newName = string.Concat(AlphaBIMUnitUtils.FeetToMm(b).ToString(), 
                "x", AlphaBIMUnitUtils.FeetToMm(h).ToString());
