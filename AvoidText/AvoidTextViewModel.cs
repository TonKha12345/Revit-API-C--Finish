...
namespace AvoidText
{
    public class AvoidTextViewModel
    {
        ...
        //Binding
        public List<Parameter> AllParameter {  get; set; } = new List<Parameter>();
        public Parameter SelectedParameter { get; set; }
        public List<PipingSystemType> AllPipingSystemType { get; set; } = new List<PipingSystemType>();
        public PipingSystemType SelectedPipingSystemType { get; set; }
        public string Value { get; set; }
        public bool IsCurrentView { get; set; }
        public bool IsSelection { get; set; }
        public bool IsEntireProject { get; set; }

        public AvoidTextViewModel(UIDocument uiDoc)
        {
            UiDoc = uiDoc;
            Doc = uiDoc.Document;
            ...
            List<Pipe> pipes = new FilteredElementCollector(Doc).OfClass(typeof(Pipe)).Cast<Pipe>().ToList();
            ParameterSet pset = pipes.FirstOrDefault().Parameters;
            foreach (Parameter p in pset)
            {
                AllParameter.Add(p);
            }
            AllParameter = AllParameter.Distinct(new CompareParameter()).ToList();
            AllParameter.OrderBy(e => e.Definition.Name).ToList();
            SelectedParameter = AllParameter.FirstOrDefault();

            //Get AllPipingSystemType 
            AllPipingSystemType = new FilteredElementCollector(Doc).OfClass(typeof(PipingSystemType)).Cast<PipingSystemType>().ToList();
            SelectedPipingSystemType = AllPipingSystemType.FirstOrDefault();
        }
        public void Resolve(Pipe pipe)
        {
            Parameter parameter = pipe.get_Parameter(BuiltInParameter.RBS_START_LEVEL_PARAM);
            ElementId levelId = parameter.AsElementId();
            ElementId systemTypeId = SelectedPipingSystemType.Id;
            ...
            {
                t.Start();
                FailureHandlingOptions option = t.GetFailureHandlingOptions();
                option.SetFailuresPreprocessor(new DeleteWarningSuper());
                t.SetFailureHandlingOptions(option);
                for (int i = 1; i < sections.Count; i++)
                {
                    ...
                    Pipe tmpPipe = Pipe.Create(Doc, systemTypeId, pipe.PipeType.Id, levelId, start, end);
                    ...
                    Doc.Create.NewElbowFitting(conn3, conn4);
                }
                ...
                if (null != startConn)
                {
                    ...
                    startPipe = Pipe.Create(Doc, pipe.PipeType.Id, levelId, startConn, sections[0].Start);
                }
                else
                {
                    // Create a pipe between pipe's start point and pipe's start section.
                    startPipe = Pipe.Create(Doc, systemTypeId, pipe.PipeType.Id, levelId, sections[0].Start, pipeLine.GetEndPoint(0));
                }
                ...
                if (null != endConn)
                {
                    ...
                    endPipe = Pipe.Create(Doc, pipe.PipeType.Id, levelId, endConn, sections[count - 1].End);
                }
                else
                {
                    // Create a pipe between pipe's end point and pipe's end section.
                    endPipe = Pipe.Create(Doc, systemTypeId, pipe.PipeType.Id, levelId, sections[count - 1].End, pipeLine.GetEndPoint(1));
                }
        ...
        private void Resolve(Pipe pipe, Section section)
        {
            ...
            using (Transaction t = new Transaction(Doc, "t"))
            {
                ...
                Pipe pipe1 = Pipe.Create(Doc, systemTypeId, pipeType.Id, levelId, start, startOffset);
                Pipe pipe2 = Pipe.Create(Doc, systemTypeId, pipeType.Id, levelId, startOffset, endOffset);
                Pipe pipe3 = Pipe.Create(Doc, systemTypeId, pipeType.Id, levelId, endOffset, end);
        ...
        private Line FindRoute(Pipe pipe, Section section)
        {
            ...
            {
                Element elem = Doc.GetElement(gref.GetReference());
        ...
        private Line FindParallelLine(Pipe pipe, Section section, Autodesk.Revit.DB.XYZ dir, double maxLength)
        {
            ...
            foreach ( ReferenceWithContext r in section.Refs)
            {
                Reference reference = r.GetReference();
                Element elem = Doc.GetElement(reference);
                if(elem is Pipe)
                {
                    step = (elem as Pipe).Diameter;
                    break;
                }
            }
